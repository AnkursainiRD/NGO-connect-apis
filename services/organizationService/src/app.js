...
export const initializeApp = async (app) => {
  try {
    logger.info('🔧 Initializing Organization Service...');
    ...
    logger.info('✅ Organization Service initialization complete');
  } catch (error) {
    logger.error('❌ Failed to initialize Organization Service:', { error: error.message, stack: error.stack });
    throw error;
  }
};
